...
interface BackendlessNoticeItem {
  objectId: string;
  title: string;
...
const PUNotices: React.FC = () => {
  ...
  useEffect(() => {
    const fetchNotices = async () => {
      try {
        const queryBuilder = Backendless.DataQueryBuilder.create().setSortBy(['date DESC']);

        const data = await Backendless.Data.of('PU_Notices').find<BackendlessNoticeItem>(queryBuilder);
