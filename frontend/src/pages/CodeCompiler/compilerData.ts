...
`
  },
  {
    ...
    extension: "asm",
    logo: "assembly",
    template: `; ── NASM x86 Assembly Demo ─────────────────────────
; Prints "Hello from NASM Assembly!" using Linux syscalls

section .data
    msg1    db  'Hello from NASM Assembly!', 0xa
    len1    equ $ - msg1

    msg2    db  'Syscall write = 4, exit = 1', 0xa
    len2    equ $ - msg2
...
function Dog:perform()
    return self.name .. " performs: " .. table.concat(self.tricks, ", ")
end

local rex = Dog.new("Rex")
rex:learn("sit"):learn("paw"):learn("roll")
print("── OOP ─────────────────────────")
print("  " .. rex:speak())
print("  " .. rex:perform())
...
table.sort(students, function(a, b) return a.gpa > b.gpa end)

local function letterGrade(g)
    if g >= 3.7 then return "A"
    elseif g >= 3.0 then return "B"
    elseif g >= 2.0 then return "C"
    else return "F" end
end
...
for _, s in ipairs(students) do
    total = total + s.gpa
    print(string.format("  %-10s gpa=%.2f [%s]", s.name, s.gpa, letterGrade(s.gpa)))
end
print(string.format("  Average GPA: %.2f", total / #students))
...
DECLARE
    ...
    -- ── Associative array ──
    TYPE t_class IS TABLE OF t_student INDEX BY PLS_INTEGER;
    v_class t_class;
    ...
    BEGIN
        ...
        ELSIF p_marks >= 60 THEN RETURN 'B';
        ELSIF p_marks >= 50 THEN RETURN 'C+';
        ELSIF p_marks >= 40 THEN RETURN 'C';
        ELSE                     RETURN 'F';
        END IF;
    END;
...
BEGIN
    -- ── Populate class ──
    v_class(1).s_name := 'Alice';   v_class(1).s_marks := 92;
    v_class(2).s_name := 'Bob';     v_class(2).s_marks := 75;
    v_class(3).s_name := 'Carol';   v_class(3).s_marks := 88;
    v_class(4).s_name := 'Dave';    v_class(4).s_marks := 61;
    v_class(5).s_name := 'Eve';     v_class(5).s_marks := 95;

    DBMS_OUTPUT.PUT_LINE('── Student Report ──────────────────');
    DBMS_OUTPUT.PUT_LINE(RPAD('Name',  10) || RPAD('Marks', 7) || 'Grade');
    DBMS_OUTPUT.PUT_LINE(RPAD('-', 22, '-'));

    FOR i IN 1 .. v_class.COUNT LOOP
        v_total := v_total + v_class(i).s_marks;
        v_count := v_count + 1;
        v_grade := get_grade(v_class(i).s_marks);
        DBMS_OUTPUT.PUT_LINE(
            RPAD(v_class(i).s_name, 10) ||
            RPAD(v_class(i).s_marks, 7) ||
            v_grade
        );
    END LOOP;
    ...
    DBMS_OUTPUT.PUT_LINE('Class Grade   : ' || get_grade(v_avg));

EXCEPTION
    WHEN ZERO_DIVIDE THEN
        DBMS_OUTPUT.PUT_LINE('Error: No students found.');
    WHEN OTHERS THEN
...
`
  },
  {
    ...
    extension: "groovy",
    logo: "groovy",
    template: `// ── Groovy Advanced Demo ────────────────────────────

// ── 1. POGO (Plain Old Groovy Object) ──
class Student {
    String name
    int age
    double gpa
    ...
    @Override
    String toString() {
        sprintf("%-10s age=%-3d gpa=%.2f [%s]", name, age, gpa, grade())
    }
}

def students = [
    new Student(name: "Alice", age: 20, gpa: 3.85),
    new Student(name: "Bob",   age: 22, gpa: 2.75),
    new Student(name: "Carol", age: 21, gpa: 3.50),
    new Student(name: "Dave",  age: 23, gpa: 1.80),
    new Student(name: "Eve",   age: 20, gpa: 3.95),
]
