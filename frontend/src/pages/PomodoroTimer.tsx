...
import { 
  ...
  Zap, Award, Calendar, ArrowLeft, ExternalLink, Sparkles, Trophy
} from "lucide-react";
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 relative z-10">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="space-y-6">
            ...
            >
              <Card className="bg-gradient-to-br from-slate-900 via-[#1e1b4b] to-purple-950 text-white shadow-premium border-0 rounded-3xl overflow-hidden relative">
                <div className="absolute right-0 top-0 w-24 h-24 bg-white/5 rounded-full blur-xl pointer-events-none" />
                <CardContent className="p-6">
                  <div className="flex items-center justify-between mb-4 border-b border-white/10 pb-2">
                    <h3 className="font-extrabold text-xs uppercase tracking-wider">Study Badges</h3>
                    <Award className="w-4.5 h-4.5 text-yellow-400" />
                  </div>
                  
                  <div className="grid grid-cols-2 gap-3.5">
                    {[
                      { title: "Focus Novice", desc: "1+ Session Done", locked: stats.totalSessions < 1, icon: Zap, color: "text-amber-400 bg-amber-400/10" },
                      { title: "Deep Focus", desc: "5+ Sessions Done", locked: stats.totalSessions < 5, icon: Target, color: "text-indigo-400 bg-indigo-400/10" },
                      { title: "Streak Master", desc: "3+ Streak Active", locked: stats.streak < 3, icon: Trophy, color: "text-emerald-400 bg-emerald-400/10" },
                      { title: "Focus Elite", desc: "10+ Sessions Done", locked: stats.totalSessions < 10, icon: Sparkles, color: "text-purple-400 bg-purple-400/10" }
                    ].map((badge, idx) => {
                      ...
                      return (
                        <div key={idx} className={`p-2.5 rounded-2xl border transition-all ${
                          ...
                          <div className="flex items-center gap-2">
                            <div className={`p-1.5 rounded-lg flex items-center justify-center ${badge.locked ? "bg-slate-800 text-slate-600" : badge.color}`}>
                              <Icon className="w-4 h-4" />
                            </div>
                            <div className="min-w-0">
                              <p className="text-[10px] font-black truncate leading-none">{badge.title}</p>
                              <p className="text-[8px] opacity-60 font-semibold truncate mt-1 leading-none">{badge.desc}</p>
                            </div>
                          </div>
                        </div>
                      );
